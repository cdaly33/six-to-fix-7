...
- **2026-05-15/2026-05-16 — Secret Handling Fixes (Branch: dev/simplify-stack-signalr-search):** Three critical fixes to Bicep and deployment guide: (1) 🔴 **Critical:** `main.bicep` bootstrapSecrets now uses separate `@secure() param sfAppPassword` for runtime `DefaultConnection` (was incorrectly using admin `sfadmin` credentials). New parameter must be injected by CI at deploy time and added to GitHub Secrets. (2) 🟡 **Warning:** Aligned Bicep KV secret names across `appservice.bicep` with canonical names that `Program.cs` actually reads (`Jwt--SigningKey`, `HubSpot--PrivateAppToken`). Removed placeholder-only secrets from bootstrapSecrets (empty slots are safer). (3) **Docs:** Updated NEXT-STEPS-FOR-CHRIS.md Step 2 to use `Read-Host -AsSecureString` for all credential values instead of inline `--value` (prevents PSReadLine history leak). Bicep validates clean. Decision archived: 2026-05-15/2026-05-16 "Tank — Secret Handling Fixes" in decisions.md. Note: Neo's Aspire integration may reference these updated deployment steps — coordinate with Neo's feature/aspire-integration branch.
