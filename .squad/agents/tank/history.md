...
- **2026-05-15 (evening) — Deployment Guide Updated for PR #20 Merge:** Chris's deployment guide (`docs/deployment/NEXT-STEPS-FOR-CHRIS.md`) now has a critical Step 0 that requires merging PR #20 before proceeding with any infrastructure steps. The guide includes: (1) Step 0 section with clear PR #20 merge workflow; (2) Updated Quick Summary table (now 4 rows: Step 0–3); (3) Before You Start section with `git log --oneline -3` verification command; (4) New Step 2 prerequisite warning about `SF_APP_PASSWORD` GitHub Secret (required for `deploy-infra.yml` to function); (5) Updated Project State Snapshot reflecting current status (sf_app/sf_admin DB role split, PeriodicTimer polling, single Search index). Branch: dev/simplify-stack-signalr-search, Commit: c5a8aa5. Critical: Chris must not skip Step 0 — all downstream infrastructure steps depend on tonight's merged fixes.
